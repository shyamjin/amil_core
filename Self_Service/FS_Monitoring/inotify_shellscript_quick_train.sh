#!/bin/bash
watchdir=/UTSAmilyAttachments/UTS_TO_AMILY/Quick_Training/
...
while : ; do
        inotifywait -m $watchdir -e CREATE|
			while read path action file; do
				...
				${PYTHON_BIN_PATH}/python -W ignore ${AMILY_SS_HOME}/atomIQ_ITSM_Self_Service-Classification_Module.pyc $watchdir"$file"
        done
done
exit 0
